...
from model_sast import SentenceClassifier, fbeta, macro_f1, evaluation, performance_table, performance_curves, \
    top_words, integrate_scores, AttentionWithContextV3
...
def get_post_tags(skills, post_type, post_tags_path, post_score_path, log_file, remove_multi_label_posts=False):
    ...
    tag_count = [(t, [tag for pid, tags in post_tags.items() for tag in tags].count(t)) for t in skills]
...
def get_train_test_posts(skills, data_path, post_tags, word_index, dataset_percent, test_size, log_file):
    all_tags = []
    last_pid_index = 0
    max_post_tags = 0
    post_id_text = {}
    ...
    print("class_count_weight of samples: {}".format(
        [(t, all_tags.count(t), len(all_tags) / all_tags.count(t) if all_tags.count(t) else 0) for t in skills]))
    log_file.write("class_count_weight of samples: {}\n".format(
        [(t, all_tags.count(t), len(all_tags) / all_tags.count(t) if all_tags.count(t) else 0) for t in skills]))
    tag_weight = {t: (len(all_tags) / all_tags.count(t)) if all_tags.count(t) else 0 for t in skills}
    class_weight = {index: tag_weight[label] for index, label in enumerate(skills)}
    print("class_weight: {}\n".format(class_weight))
    ...
    for num_tags in range(1, max_post_tags + 1):
        ...
        sample_tag_posts = {t: [] for t in skills}
        ...
        print("num of posts for each tag:{}".format({t: len(sample_tag_posts[t]) for t in skills}))
        log_file.write("num of posts for each tag:{}\n".format(
            {t: len(sample_tag_posts[t]) for t in skills}
        ))
...
def get_inputs_for_multi_label(skills, post_id_text, word_index, post_tags, train_posts, test_posts,
                               add_single_cls_token, max_sentence_len, log_file):
    multilabel_binarizer = MultiLabelBinarizer()
    multilabel_binarizer.fit([skills])
...
def get_inputs_for_multi_label_2(skills, post_id_text, word_index, post_tags, train_posts, test_posts,
                                 add_single_cls_token, max_sentence_len, log_file):
    multilabel_binarizer = MultiLabelBinarizer()
    multilabel_binarizer.fit([skills])
    labels_names = multilabel_binarizer.classes_
    ...
    train_tag_pid = {}
    for pid in train_posts:
        representative_tag = min([skills.index(tag) for tag in post_tags[pid]])
...
def train(dataset, data_path, skill_cluster_path, post_tags_path, post_score_path, result_path, dataset_percent,
          test_size, post_type, num_words, word_vector_dim, train_embedding, max_sentence_len, epochs, batch_size,
          continue_training, initial_epoch, num_top_trans, lstm_dim, dropout_value, word_index_dir,
          use_class_weight, split_data_for_validation):
    ...
    skills = sorted(list(tag_skill.keys()))
    post_tags = get_post_tags(skills, post_type, post_tags_path, post_score_path, log_file, False)
    ...
    train_posts, test_posts, post_id_text, post_index_id, class_weight = get_train_test_posts(
        skills, data_path, post_tags, word_index, dataset_percent, test_size, log_file
    )

    if split_data_for_validation:
        x_train, x_val, x_test, y_train, y_val, y_test, labels_names = get_inputs_for_multi_label_2(
            skills, post_id_text, word_index, post_tags, train_posts, test_posts, True, max_sentence_len, log_file
        )
    else:
    ...
    if continue_training:
        model = tensorflow.keras.models.load_model(
            ...
            custom_objects={
                "macro_f1": macro_f1, "fbeta": fbeta, "AttentionWithContextV3": AttentionWithContextV3,
    ...
    integrate_scores(labels_names, log_file, result_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', help="", required=True, default="SAST")
    ...
    parser.add_argument('--split_data_for_validation', help="", action='store_true')
    args = parser.parse_args()

    train(args.dataset, args.data_path, args.skill_cluster_path, args.post_tags_path, args.post_score_path,
          args.result_path, args.dataset_percent, args.test_size,
          args.post_type, args.num_words, args.word_vector_dim, args.train_embedding, args.max_sentence_len,
          args.epochs, args.batch_size, args.continue_training, args.initial_epoch, args.num_top_trans,
          args.lstm_dim, args.dropout_value, args.word_index_dir, args.class_weight, args.split_data_for_validation)
