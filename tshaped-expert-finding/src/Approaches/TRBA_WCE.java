        ...
        for (String model_type : Arrays.asList("TRBA1", "TRBA2")) {
            for (int t : Arrays.asList(1, 2, 3, 4, 5)) {
                try {
                    ...
                    ResultPath = "./files/Result/" + dataSetName + "/" + model_type + "_WCE/top" + t + "/";
            ...
            for (String skillArea : skillAreas) {
                ...
                for (String sa : skillAreas) {
                    ...
                    if (sa.equalsIgnoreCase(skillArea)) {
                        if (prob != 0.0)
                            bce += -0.5 * (Math.log10(prob) / Math.log10(2)); // -0.5 * log_2(q_i)
                        else
                            bce += -0.5 * (Math.log10(1e-10) / Math.log10(2)); // -0.5 * log_2(q_i)
                    } else {
                        if (prob != 1.0)
                            bce += -0.5 * (Math.log10(1 - prob) / Math.log10(2)) / (skillAreas.size() - 1); // -log_2(1-q_i)
                        else
                            bce += -0.5 * (Math.log10(1e-10) / Math.log10(2)) / (skillAreas.size() - 1); // -log_2(1-q_i)
                    }
                }
...
//        writer.close();
    ...
    private LinkedHashMap<String, LinkedHashMap<Integer, Double>> calculateProbabilityTandRGivenExpertAndSA(
        ...
        for (Map.Entry<String, LinkedHashMap<Integer, Double>> SAItem : SA__UId_ProbabilityRGivenExpertAndSA.entrySet()) {
            ...
            PrintWriter writer = new PrintWriter(ResultPath + skillArea + "_WCE_Probability.txt");
