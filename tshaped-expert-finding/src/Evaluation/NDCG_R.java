...
public class NDCG_R {
    ...
    public NDCG_R(String approachPath, String tagsOfSkillAreaPath, String skillShapesXMLPath, int rank) throws IOException {
        ReadClusteringFile(tagsOfSkillAreaPath);
        CalculateRealData(skillShapesXMLPath);
        calculateAvgNDCG(approachPath, rank);
        PrintNDCG(approachPath, rank);
    }
    ...
    public void calculateAvgNDCG(String approachPath, int rank) throws FileNotFoundException, IOException {
        double NDCGSum = 0;
        for (String skillArea : SAList) {
            double NDCG = calculateNDCG(skillArea, approachPath, rank);
    ...
    private double calculateNDCG(String skillArea, String approachPath, int rank) throws FileNotFoundException, IOException {
        ...
        reader.close();
        double IDCG = CalculateIDCG(skillArea, rank);
        double NDCG = DCG / IDCG;
        return NDCG;
    }

    private double CalculateIDCG(String SkillArea, int rank) {
        LinkedHashMap<String, Integer> shape_count = realData.get(SkillArea);
        int TCount = shape_count.getOrDefault("T", 0);
        int CCount = shape_count.getOrDefault("C", 0);
        int NonExpertCount = shape_count.getOrDefault("NonExpert", 0);

        if (rank <= TCount)
            return CalculateIDCGForShape("T", 1, rank); // IDCGForTs

        double IDCGForTs = CalculateIDCGForShape("T", 1, TCount);
        int new_rank = rank - TCount;
        if (new_rank <= CCount){
            double IDCGForCs = CalculateIDCGForShape("C", TCount + 1, TCount + new_rank);
            return IDCGForTs + IDCGForCs;
        }

        double IDCGForCs = CalculateIDCGForShape("C", TCount + 1, TCount + CCount);
        int new_rank_2 = new_rank - CCount;
        double IDCGForNonExperts = CalculateIDCGForShape("NonExpert", TCount + CCount + 1, TCount + CCount + Math.min(new_rank_2, NonExpertCount));
    ...
    public void PrintNDCG(String approachPath, int rank) throws FileNotFoundException, IOException {
        PrintWriter writer = new PrintWriter(approachPath + "NDCG_" + rank + ".csv");
